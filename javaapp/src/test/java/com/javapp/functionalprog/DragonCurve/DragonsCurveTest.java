package com.javapp.functionalprog.DragonCurve;
...
public class DragonsCurveTest {
  ...
  public void setup() {
    ...
    IntStream.range(0, ns.size())
        .forEach(i -> tests.put(ns.get(i), curves.get(i))); //Poor man's zip
  }
  ...
  public void testMapFunction() throws Exception {
    ...
    IntStream.range(0, ins.size())
        .forEach(i -> inOut.put(ins.get(i), outs.get(i))); //Poor man's zip
    inOut.forEach((i, o) -> {
      String res = testee.mapFunction.apply(i);
      assertEquals("Input to map function " + i + " (char '" + (char) i.intValue() + "') response ",
          o, res);
    });
  }
